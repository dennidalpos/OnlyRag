...
|---|---|---|---|---|---|---|---|
| Bootstrap prerequisites | `scripts\Bootstrap-Prerequisites.ps1` | Verify Windows development prerequisites, restore .NET packages, install web dependencies, and prepare optional OCR runtime. | Fresh checkout setup or dependency repair. | Manual. | Windows, PowerShell 7, .NET 10 SDK; Node/npm unless `-SkipNode`; optional Python/Ollama/LibreOffice. | `%LOCALAPPDATA%\OnlyRag`, restored packages, `src\OnlyRag.Web\node_modules`, optional OCR env. | Does not build, package, sign, install, deploy, or release. |
