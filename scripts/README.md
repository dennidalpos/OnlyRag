...
| Build installer | `scripts\Build-Installer.ps1` | Build web assets, prepare Qdrant, publish self-contained `win-x64` app, verify native runtime payloads, and compile the Inno Setup installer. | Packaging candidate build. | `Invoke-Gate.ps1 -IncludeInstaller`, `Sign-Release.ps1`. | .NET 10 SDK, Node/npm, Inno Setup 6, installer brand assets; network access when Qdrant payload is missing. | `packaging\qdrant\payload`, `artifacts\publish\OnlyRag\win-x64`, `artifacts\installer\*.exe`. | Fails if DirectML/ONNX Runtime native files are missing. `-SigningCertificateThumbprint` signs the installer during build; unsigned output is not release-ready. |
