...
public sealed class OcrProvisionRuntimeResolverTests
{
    ...
    [Theory]
    [InlineData("596.49, NVIDIA GeForce RTX 2070", "cuda129", "requirements-nvidia-cu129.txt")]
    [InlineData("561.17, NVIDIA GeForce RTX 4070", "cuda126", "requirements-nvidia-cu126.txt")]
    [InlineData("520.06, NVIDIA GeForce GTX 1660", "cuda118", "requirements-nvidia-cu118.txt")]
    ...
    public async Task OldNvidiaDriverFallsBackToCpuForAuto()
    {
        FakeProcessLauncher processLauncher = new()
        {
            RunHandler = (_, _, _) => new LocalProcessResult(0, "519.99, NVIDIA GeForce GTX 1080", string.Empty)
        };
        OcrProvisionRuntimeResolver resolver = new(processLauncher, _ => @"C:\Windows\System32\nvidia-smi.exe");
