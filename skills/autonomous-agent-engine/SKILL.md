...
- **Durable Persistent Run State Machine** (`PersistentAgentRunStateMachine`): Persisted in SQLite schema v9 (`agent_runs`, `agent_run_transitions`). Enforces the strict phase sequence: `Plan` → `Act` → `Observe` → `Verify` → `Recover` → `Finalize`.
