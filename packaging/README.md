...
- Select Additional Tasks page, using the localized Inno Setup `CreateDesktopIcon` and `AdditionalIcons` messages for the desktop shortcut;
...
| OCR bridge scripts, setup preinstall script, CPU/NVIDIA requirements, and runtime manifest | Optional OCR provisioning and OCR runtime bridge | Included from `scripts\ocr` | `Test-OnlyRagPublishPayload`; automatic installer OCR setup; Settings > Configura OCR reports missing bridge/requirements/manifest |
...
| Python 3.10 through 3.13 plus PaddleOCR packages | Optional OCR for scanned PDFs/images | Installer setup prepares a per-user venv when Python and Internet access are available; Settings/bootstrap can repair it | Automatic installer OCR setup, Settings > Configura OCR, and OCR availability check |
| NVIDIA driver plus pinned PaddlePaddle GPU wheel | Optional OCR acceleration | Automatic `auto` provisioning chooses CUDA 12.9, 12.6, or 11.8 when diagnostics resolve a compatible local NVIDIA path; otherwise CPU is used | Installer ready memo, OCR provisioning status, startup auto-enable, and bridge GPU check |

External and configurable:

- Ollama is not bundled. Install and configure it separately for model features.
- OCR/PaddleOCR runtime packages and OCR models are optional and are not bundled in the installer. The supported strategy is a per-user Python virtual environment under `%LOCALAPPDATA%\OnlyRag\ocr-python`, prepared by automatic installer setup, **Configura OCR** in Settings, or the developer bootstrap from pinned `scripts\ocr\requirements.txt` versions and verified against `scripts\ocr\runtime-manifest.json`. GPU OCR uses separate pinned NVIDIA requirement files for CUDA 12.9, CUDA 12.6, or CUDA 11.8 and is installed only when automatic provisioning resolves a compatible local NVIDIA driver.
